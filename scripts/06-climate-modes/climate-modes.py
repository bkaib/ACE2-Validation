...
def compute_nao_index(da, n_modes=5):
    ...
    eofs_hpa = model.components(normalized=False)  # Shape: (mode, lat, lon)
...
def compute_pna_index(
        da,
        lon_min = 120, lon_max = 300, lat_min = 20, lat_max = 90, 
        n_modes=5,
        ):
    # Select DJF months only
    da = da.sel(time=da.time.dt.month.isin([12,1,2]))
    ...
    # 4. Retrieve PCs (Index Time Series)
    # Normalized to unit variance (sigma = 1) -> Unitless PNA Index
    pcs_unitless = model.scores(normalized=True)  # Shape: (time, mode)
...
def main():
    ...
    compare_climate_mode_nao(ace2_pca_metrics, era5_pca_metrics, mode=1, file="nao-index-comparison")
